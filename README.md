...
`figures/README.md` for how to render each figure. `slow_full_flow.parquet` and
the per-scenario HBV output CSVs are too large to commit and are regenerated by
the pipeline; small copies of `projection_flow.parquet` and
`forcing_deltas_rcp85.csv` *are* committed under `derived_output/`, so only
Figures 2 and 3 require a regenerated transition table.
